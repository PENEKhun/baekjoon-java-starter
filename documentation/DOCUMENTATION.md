...
한번 실행이 된 이후로, jar파일이 있는 경로에 config.json이 생성됩니다. 이 파일을 수정하여 설정을 커스터마이징 할 수 있습니다.
> ⚠️ 단순 실행으로 설정파일이 생성되지 않습니다. 적당한 문제 번호를 입력하여 프로그램 실행을 한사이클 진행해주세요. (ex: 1000) 해당 사항은 추후 수정될
> 예정입니다.
